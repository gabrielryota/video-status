...
function FooterBanner({ card, areaName }) {
  ...
  let deliveryText = "—";
  if (overall === 100) {
    deliveryText = "✅ Concluído!";
  } else if (deadline) {
    const d = deadline.estimated;
    deliveryText = d.toLocaleDateString("pt-BR", { day: "2-digit", month: "long", year: "numeric" });
  } else if (card.dueDate) {
    deliveryText = new Date(card.dueDate + "T12:00:00").toLocaleDateString("pt-BR", { day: "2-digit", month: "long", year: "numeric" });
  }
    ...
    <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#0a0a16", borderTop: `1px solid ${color}33`, zIndex: 500, overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1440, margin: "0 auto", padding: "14px 24px", display: "flex", alignItems: "center", gap: 24, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: color + "22", border: `1px solid ${color}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>
            {activeStage.icon}
          </div>
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 700, color: "#ccc", fontFamily: "'Syne',sans-serif", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 220 }}>{card.title}</div>
          </div>
        </div>

        {/* Divider */}
        <div style={{ width: 1, height: 32, background: "#1a1a2e", flexShrink: 0 }} />

        {/* Big message */}
        <div style={{ flex: 1, minWidth: 200 }}>
          <span style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(13px,2vw,17px)", color: "#fff", letterSpacing: "-.02em" }}>
            SEU VÍDEO ESTÁ{" "}
            <span style={{ color, textShadow: `0 0 20px ${color}` }}>{overall}% PRONTO</span>
            {"  ·  "}
            <span style={{ color: "#888", fontWeight: 600, fontSize: "clamp(11px,1.5vw,14px)" }}>
              PREVISÃO DE ENTREGA:{" "}
              <span style={{ color: overall === 100 ? "#2ED573" : "#ccc" }}>{deliveryText}</span>
            </span>
          </span>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 14px", background: activeStage.color + "15", border: `1px solid ${activeStage.color}33`, borderRadius: 20, flexShrink: 0 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 800, color: color, fontFamily: "'Syne',sans-serif" }}>{calcStageProgress(card, activeStage.id)}%</span>
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try { const r = JSON.parse(localStorage.getItem(STORAGE_KEY)); if (r?.value) setCards(JSON.parse(r.value)); } catch (_) {}
      setLoaded(true);
    })();
  }, []);
