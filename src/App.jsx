...
function VideoCard({ card, onEdit, onDelete }) {
  ...
  return (
    <div onClick={() => onEdit && onEdit(card)}
      style={{ background: "#0d0d1a", border: "1px solid #16162a", borderRadius: 14, padding: "15px", marginBottom: 10, cursor: onEdit ? "pointer" : "default", position: "relative", overflow: "hidden", transition: "transform .15s, box-shadow .15s, border-color .15s" }}
      onMouseOver={(e) => { if (!onEdit) return; e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.borderColor = color + "55"; e.currentTarget.style.boxShadow = `0 8px 28px rgba(0,0,0,.5)`; }}
